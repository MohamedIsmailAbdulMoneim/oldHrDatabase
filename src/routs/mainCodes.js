...
function getEmpsDetails(req, res, next) {
    ...
    e.NAME_ARABIC,
    e.EMPLOYEE_ID,
    DATE_FORMAT(e.SECTOR_JOIN_DATE, '%yyyy/%mm/%dd') as SECTOR_JOIN_DATE,
    dateofj.TRANS_DATE,
...
OR NULL) AS genderar,
(SELECT STATUS_ARABIC FROM military_service_status WHERE e.MILITARY_SERVICE_STATUS = military_service_status.MILITARY_SERVICE_STATUS) as milstatus,
(
    SELECT
        RELIGION_NAME
    FROM
...
function getEmpDetails(req, res, next) {
    ...
    let query = `
        ...
        e.EMPLOYEE_ID,
        e.NAME_ARABIC,
        DATE_FORMAT(e.NATIONAL_ID_CARD_EXPIRE_DATE, '%Y/%m/%d') as NATIONAL_ID_CARD_EXPIRE_DATE,
        DATE_FORMAT(e.SECTOR_JOIN_DATE, '%Y/%m/%d') as SECTOR_JOIN_DATE,
        ...
        (SELECT CONTRACT_TYPE_ARABIC FROM contract_type WHERE e.CONTRACT_TYPE = contract_type.CONTRACT_TYPE) as contacttype,
        (
        SELECT
            GOVERNORATE_ARABIC
        FROM
    ...
    SELECT
        ...
        a_sup_box.box_card,
        a_sup_box.MAIN_BOX_ID,
        date_format(a_job_trans.TRANS_DATE, '%Y/%m/%d') as curjobdate,
        a_job_trans.JOB_ASSIGNMENT_FORM,
    ...
    SELECT
    date_format(TRANS_DATE, '%Y/%m/%d') as TRANS_DATE_First
FROM
    a_job_trans
WHERE
